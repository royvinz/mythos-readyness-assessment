...
function renderQuestionCard(q, pillar) {
  ...
  return `<div class="q-card" id="card-${esc(q.id)}"><div class="q-meta"><span class="q-id">${esc(q.id)}</span>${csaBadge}${nistBadge}${atlasBadge}${owaspBadge}${cascBadge}</div><div class="q-text">${esc(lang.q)}</div>${lang.note?`<div class="q-cadrage">${esc(lang.note)}</div>`:""}${otherRef}<div class="score-cards-grid">${scoreCards}</div><div class="q-note"><label class="q-note-label" for="note-${esc(q.id)}">${t("q_note_label")}</label><textarea class="q-note-input" id="note-${esc(q.id)}" placeholder="${t("q_note_ph")}" rows="2" oninput="setNote('${esc(q.id)}',this.value)" aria-label="${t("q_note_label")}">${esc(note)}</textarea></div></div>`;
}
...
function buildIngestPrompt(planText) {
  const questions=DATA.questions.map(q=>({id:q.id,pillar:q.pillar,sub:q.sub,question:q.en.q+(q.en.note?" (Note: "+q.en.note+")":""),levels:{0:q.en.l0,1:q.en.l1,2:q.en.l2,3:q.en.l3}}));
...
async function loadAndImportCSV(file) { const text=await file.text();const res=parseAndImportCSV(text);renderAll();const s=document.getElementById("import-status");if(s) s.textContent=importStatusMessage(res); }
async function decryptAndImport(file,pass) { const enc=new TextEncoder();const text=await file.text();const obj=JSON.parse(text);const b64d=s=>Uint8Array.from(atob(s),c=>c.charCodeAt(0));const salt=b64d(obj.salt),iv=b64d(obj.iv),ct=b64d(obj.ciphertext);const km=await crypto.subtle.importKey("raw",enc.encode(pass),"PBKDF2",false,["deriveKey"]);const key=await crypto.subtle.deriveKey({name:"PBKDF2",salt,iterations:200000,hash:"SHA-256"},km,{name:"AES-GCM",length:256},false,["decrypt"]);const plain=await crypto.subtle.decrypt({name:"AES-GCM",iv},key,ct);const res=parseAndImportCSV(new TextDecoder().decode(plain));renderAll();const s=document.getElementById("import-status");if(s) s.textContent=importStatusMessage(res); }
// v1.3.1 — Version-tolerant import: only ids present in the current questionnaire are imported
// (answers keyed to unknown / removed ids are skipped); remaining unanswered questions are reported
// so an assessment exported from an older questionnaire version resumes cleanly.
function importStatusMessage(res) { const remaining=DATA.questions.filter(q=>SCORES[q.id]===undefined||SCORES[q.id]===null).length; let msg=t("import_success")+" ("+res.count+" questions)"; if(remaining>0) msg+=" · "+remaining+" "+t("import_remaining"); if(res.skipped>0) msg+=" · "+res.skipped+" "+t("import_skipped"); return msg; }
function parseAndImportCSV(csvText) { const known=new Set(DATA.questions.map(q=>q.id));const text=csvText.replace(/^\uFEFF/,"");const lines=text.split("\n").filter(l=>l.trim());if(lines.length<2) return {count:0,skipped:0};let count=0,skipped=0;for(let i=1;i<lines.length;i++){const cols=parseCSVLine(lines[i]);if(!cols||cols.length<6) continue;const id=cols[0],scoreStr=cols[5],note=cols[7]||"";if(!id||scoreStr==="") continue;if(!known.has(id)){skipped++;continue;}const score=parseInt(scoreStr);if(!isNaN(score)&&score>=0&&score<=3){SCORES[id]=score;count++;}if(note) NOTES[id]=note;}saveStorage();return {count,skipped}; }
